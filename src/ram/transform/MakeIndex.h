...
public:
    ...
    ExpressionPair getExpressionPair(const Constraint* binRelOp, std::size_t& element, const std::optional<std::size_t>& identifier);
    ExpressionPair getLowerUpperExpression(
            Condition* c, std::size_t& element, const std::optional<std::size_t>& level, RelationRepresentation rep);
    ...
    Own<Condition> constructPattern(const std::vector<std::string>& attributeTypes, RamPattern& queryPattern,
            bool& indexable, VecOwn<Condition> conditionList, std::size_t identifier, RelationRepresentation rep);
