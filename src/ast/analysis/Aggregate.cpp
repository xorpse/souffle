...
std::set<std::string> getLocalVariables(const TranslationUnit& tu, const Clause& clause, const Aggregator& aggregate) {
    ...
    visitDepthFirst(aggregate, [&](const Variable& v) {
        allVariablesInAggregate.insert(v.getName());        
    });
...
std::set<std::string> getVariablesOutsideAggregate(const Clause& clause, const Aggregator& aggregate) {
        ...
        visitDepthFirst(clause, [&](const Variable& var) {
            variableOccurrences[var.getName()]++;        
        });
        visitDepthFirst(aggregate, [&](const Variable& var) {
            variableOccurrences[var.getName()]--;
        });
...
std::string findUniqueVariableName(const Clause& clause, std::string base) {
    ...
    visitDepthFirst(clause, [&](const Variable& v) {
        variablesInClause.insert(v.getName());        
    });
    int varNum = 0;
...
std::set<std::string> getInjectedVariables(const TranslationUnit& tu, const Clause& clause, const Aggregator& aggregate) {
     ...
     *   
     *   2. A(letter, x) :- a(letter, _), x = min y : { a(l, y), y < max y : { a(letter, y) } }, z = aggr_var_0.
     *
     ...
     *
     *   Now, as long as we recorded all of the variables that occurred inside the aggregate, the intersection of these two sets (variables in an outer scope)
     *   and (variables occurring in t
     **/

    /**
     *  A variable is considered *injected* into an aggregate if it occurs both within the aggregate AS WELL AS
     *  *grounded* in the outer scope. An outer scope could be an outer aggregate, or also just the base/rule scope. 
     *  (i.e. not inside of another aggregate that is not an ancestor of this one).
     *
     *  So then in order to calculate the set of variables that have been injected into an aggregate, we perform
     *  the following steps:
     *
     ...
     *      2a. Replace all non-ancestral aggregates with variables so that we can ignore their variable contents (it is not relevant).
     *      2b. Replace the target aggregate with a variable as well
     *      2c. Ground all occurrences of the non-ancestral aggregate variable replacements (i.e. grounding_atom(+aggr_var_0))
     *      2d. Visit all variables occurring in this tweaked clause that we made from steps 2a-2c, and check if they are grounded, and also
     *          occur in the set of target aggregate variables. Then it is an injected variable.
     **/
    // Step 1
    std::set<std::string> variablesInTargetAggregate;
    visitDepthFirst(aggregate, [&](const Variable& variable) {
        variablesInTargetAggregate.insert(variable.getName());        
    });
    ...
    struct ReplaceAggregatesWithVariables : public NodeMapper {
            // Variables introduced to replace aggregators
            mutable std::set<std::string> aggregatorVariables;
            std::set<Own<Aggregator>> ancestors; // ancestors are never replaced so don't need to clone, but actually
            // it will have a child that will become invalid at one point. This is a concern. Need to clone these bastards too.
            Own<Aggregator> targetAggregate; // make sure you clone the agg first.
            ...
            std::unique_ptr<Node> operator()(std::unique_ptr<Node> node) const override {
                ...
                if (auto* aggregate = dynamic_cast<Aggregator*>(node.get())) {
                    ...
                    if (!isAncestor || *aggregate == *targetAggregate) {
                        ...
                        if (!isAncestor) {
                            aggregatorVariables.insert(newVariableName.str()); // we don't want to ground the target aggregate (why?) because these variables oculd be injected and that is fine.
                            // but we are not supposed to be judging the aggregate as grounded and injected into the aggregate, that wouldn't make sense
                        }
    ...
    for (const auto& argPair : analysis::getGroundedTerms(tu, *tweakedClause)) {
        if (const auto* variable = dynamic_cast<const Variable*>(argPair.first)) {
            ...
            if (varIsGrounded && variablesInTargetAggregate.find(variable->getName()) != variablesInTargetAggregate.end()) {
